...
import { FileUp, Loader2, X, OctagonX, FileText, ArrowLeft, ListChecks, ChevronDown, ChevronUp, Bot, CheckCircle2, Sparkles, AlertCircle, Send, MessageSquare, User, Trash2, Download } from "lucide-react";
...
const SK = "sources-sought-evaluator";
...
const SourcesSoughtEvaluator = () => {
  ...
  const runAgent = useCallback(async () => {
    if (files.length === 0) {
      toast({ title: "Missing documents", description: "Upload at least one Sources Sought document first.", variant: "destructive" });
      return;
    }
    ...
    try {
      const resp = await fetch(
        `${import.meta.env.VITE_FUNCTIONS_URL || import.meta.env.VITE_SUPABASE_URL}/functions/v1/sources-sought-agent`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({ rfpDocuments, supplementaryDocument, proposalType: "government", cloudProvider: "general", sessionId, preExtractedRequirements, sectionHeadings }),
        ...
        for (const part of parts) {
          ...
          switch (event.type) {
                ...
                try {
                  ...
                  if (userData?.user) {
                    ...
                    const { data: diagramData } = await supabase
                      .from("diagrams")
                      .insert({ user_id: userData.user.id, title: result.solutionTitle || "Sources Sought Response Structure", nodes: diagramNodes as any, edges: diagramEdges as any, source: "sources-sought-evaluator" })
                      .select("id").single();
            ...
            case "agent_done":
              ...
              toast({ title: "Assessment complete!", description: "Sources Sought capability assessment is ready." });
              break;
  ...
  }, [files, supplementaryFile, sessionId, toast]);
    ...
    try {
      const resp = await fetch(
        `${import.meta.env.VITE_FUNCTIONS_URL || import.meta.env.VITE_SUPABASE_URL}/functions/v1/generate-report`,
        {
          ...
          body: JSON.stringify({ projectName, proposalType: "government", cloudProvider: "general", requirementsResult, evaluationResult, solutionResult }),
        }
      );
      ...
      if (win) {
        ...
        a.href = url;
        a.download = `sources-sought-evaluation-${projectName}.html`;
        a.click();
  ...
  }, [requirementsResult, evaluationResult, solutionResult, projectName, toast]);
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="font-heading text-2xl font-bold text-foreground">Sources Sought Evaluator</h1>
            <p className="text-muted-foreground mt-1">
              Evaluate your company's capability match against a Sources Sought notice before writing a response.
            </p>
          </div>
          {(requirementsResult || evaluationResult || solutionResult) && (
            <Button
              ...
              onClick={() => {
                ["sessionId","requirementsResult","evaluationResult","solutionResult","diagramId","agentLog"].forEach(k => localStorage.removeItem(`${SK}:${projectName}:${k}`));
                setRequirementsResult(null);
                setEvaluationResult(null);
                setSolutionResult(null);
                setDiagramId(null);
        ...
        <div className="space-y-6">
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label>Capability Statement (optional)</Label>
                  <div
                    ...
                    {supplementaryFile ? (
                      ...
                      <>
                        ...
                        <p className="text-xs text-muted-foreground">
                          Upload your capability statement for compliance assessment
                        </p>
                      </>
                    )}
                    <input
            ...
            <Card>
              <CardHeader className="pb-4">
                <CardTitle className="text-base">Sources Sought Documents</CardTitle>
                <CardDescription>Upload Sources Sought notices, RFIs, or market research documents</CardDescription>
              </CardHeader>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Automatically runs requirements extraction, capability assessment, and response structure generation in sequence.
                  </p>
                </div>
              </div>
              <Button
          ...
          {(indexedChunks !== null || files.some(f => f.status === "done")) && (
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription>
                  Ask follow-up questions about the uploaded Sources Sought notice and capability documents.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3 pt-0">
                ...
                <div className="flex gap-2 items-end">
                  <Textarea
                    ref={chatInputRef}
                    placeholder="Ask a question about the Sources Sought notice… e.g. 'What NAICS code is requested?' or 'What past performance is required?'"
                    value={chatInput}
...
export default SourcesSoughtEvaluator;
