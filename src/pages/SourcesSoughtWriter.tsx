...
import {
  ...
  LayoutTemplate, Database, Bot,
  AlertCircle, ListChecks, Library, Pencil, Eye,
} from "lucide-react";
...
const SK = "sources-sought-writer";
...
// ─── Sources Sought Writer ────────────────────────────────────────────────────
const SourcesSoughtWriter = () => {
  ...
  const [selectedPalette, setSelectedPalette] = useState<PaletteId>("navy-ivory");
  const [rfpFile,         setRfpFile]         = useState<UploadedFile | null>(null);
  const [capFiles,        setCapFiles]        = useState<UploadedFile[]>([]);
  ...
  const handleRfpUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast({ title: "Error reading Sources Sought document", variant: "destructive" });
    }
  ...
  const runAgent = useCallback(async () => {
    if (!rfpFile || rfpFile.status !== "done") {
      toast({ title: "Sources Sought document required", description: "Upload the Sources Sought notice before running the agent.", variant: "destructive" });
      return;
    }
    ...
    appendLog({
      ...
      message: `Scanning Sources Sought notice — found ${preExtracted.filter(r => r.type === "shall").length} SHALL and ${preExtracted.filter(r => r.type === "must").length} MUST information requests.`,
    });
    ...
    const MAX_PASSES = 10;
    const FUNC_URL = `${import.meta.env.VITE_FUNCTIONS_URL || import.meta.env.VITE_SUPABASE_URL}/functions/v1/write-sources-sought`;
    ...
    try {
      ...
      for (let pass = 2; pass <= MAX_PASSES && !done; pass++) {
        appendLog({ type: "tool_start", tool: "write_proposal",
          message: `Continuing Sources Sought response — pass ${pass} of up to ${MAX_PASSES} (${outlineSections.length} sections planned)...` });
      ...
      if (!done) {
        appendLog({ type: "agent_done", message: `Response generation reached ${MAX_PASSES} passes.` });
      }
      toast({ title: "Sources Sought response complete!", description: "Full response generated successfully." });
  ...
  // ── Update / Modify existing response ─────────────────────────────────────
  const updateProposal = useCallback(async () => {
    ...
    const effectiveInstructions = hasUserEdits
      ? `CRITICAL — USER EDITS PROTECTION: This response contains manual edits made directly by the user. You MUST preserve ALL existing text exactly as written. Do NOT rephrase, reword, or restructure any existing content unless it is explicitly named in the update instructions below. Only apply the specific changes requested.\n\nUPDATE INSTRUCTIONS:\n${modificationPrompt.trim()}`
      : modificationPrompt.trim();
    setProposal("");
    appendLog({ type: "tool_start", tool: "write_proposal",
    ...
    try {
      ...
      for (let pass = 2; pass <= MAX_MOD_PASSES && !done; pass++) {
        appendLog({ type: "tool_start", tool: "write_proposal",
          message: `Continuing modified response — pass ${pass} of up to ${MAX_MOD_PASSES}...` });
      ...
      setHasUserEdits(false);
      toast({ title: "Response updated!", description: "Modifications applied successfully." });
    } catch (e: any) {
      if (e.name === "AbortError") {
  ...
  const downloadAsWord = useCallback(async () => {
    ...
    paragraphs.push(new Paragraph({ children: [new TextRun({ text: "Sources Sought Response", font: "Calibri", size: 34, color: sHex, italics: true })], alignment: AlignmentType.CENTER, spacing: { after: 120 } }));
    ...
    paragraphs.push(new Paragraph({ children: [new TextRun({ text: "MARKET RESEARCH RESPONSE — NOT A PROPOSAL", font: "Calibri", size: 18, color: "999999", italics: true })], alignment: AlignmentType.CENTER, spacing: { after: 800 } }));
    ...
    saveAs(blob, `${(companyName || "sources-sought").replace(/\s+/g, "_")}_response.docx`);
    toast({ title: "Downloaded!", description: "Sources Sought response saved as Word document." });
  ...
  const downloadAsPdf = useCallback(async () => {
    ...
    doc.text("Sources Sought Response", pageW / 2, y, { align: "center" });
    y += 22;
    ...
    y += 14;
    doc.text("MARKET RESEARCH RESPONSE — NOT A PROPOSAL", pageW / 2, y, { align: "center" });
    doc.addPage();
    y = marginT;
    ...
    doc.save(`${(companyName || "sources-sought").replace(/\s+/g, "_")}_response.pdf`);
    toast({ title: "Downloaded!", description: "Sources Sought response saved as PDF." });
  }, [proposal, companyName, palette, logoDataUrl, toast]);
    ...
    <SparkLayout>
      <div className="max-w-7xl mx-auto space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-heading text-2xl font-bold text-foreground">AI Sources Sought Writer</h1>
            <p className="text-muted-foreground mt-0.5 text-sm">
              Agentic Sources Sought response generation — indexes your capability documents, extracts every information request, then writes a complete concise response.
            </p>
          </div>
        </div>
          ...
          <div className="w-72 shrink-0 space-y-4">
            ...
            <Card>
              <CardHeader className="pb-3">
                ...
                <CardDescription className="text-[11px]">Sources Sought notice required. Capability docs build the knowledge base.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <UploadZone
                  label="Sources Sought Notice *"
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    ...
                    <p className="text-xs text-muted-foreground">Capability statement, past performance, certifications...</p>
                ...
                <div className="pt-1 border-t border-border/50">
                  ...
                  <p className="text-[10px] text-muted-foreground mt-1">
                    Saves company docs as permanent style templates for all future responses.
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="flex flex-col h-full" style={proposal ? { borderColor: palette.border } : {}}>
              <CardHeader className="pb-3 flex-row items-center justify-between shrink-0">
                <div className="flex items-center gap-2">
                  <CardTitle className="text-sm">Generated Sources Sought Response</CardTitle>
                  {proposal && (
                ...
                {proposal && !isAgentRunning && (
                  <div className="flex gap-2 items-center">
                    <Button
                      ...
                      title={isEditing ? "Switch to preview" : "Edit response text directly"}
                    >
                      {isEditing ? (
                ...
                {!proposal && !isAgentRunning && !isModifying ? (
                  <div className="h-[calc(100vh-250px)] min-h-[500px] flex items-center justify-center text-muted-foreground">
                    <div className="text-center space-y-3">
                      ...
                      <p className="text-sm font-medium">Your Sources Sought response will stream here</p>
                      <p className="text-xs text-muted-foreground max-w-xs">
                        Upload your Sources Sought notice and capability documents, then click <strong>Run Agent</strong>. The agent will extract every information request and write a complete concise response.
                      </p>
                    </div>
                  </div>
                ) : (
                  ...
                  <ScrollArea className="h-[calc(100vh-300px)] min-h-[420px] pr-2">
                    ...
                    {proposal && (
                      <div className="rounded-xl mb-6 p-5 text-center space-y-1.5 border" style={{ background: palette.bg, borderColor: palette.border }}>
                        ...
                        <p className="text-sm font-medium" style={{ color: palette.secondary }}>Sources Sought Response</p>
                        <p className="text-[10px] text-muted-foreground pt-1">
                          {new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })} · MARKET RESEARCH RESPONSE
                        </p>
                      </div>
                    )}
                  ...
                  {proposal && !isAgentRunning && !isModifying && (
                    <div className="pt-3 mt-2 border-t border-border/50 space-y-2 shrink-0">
                      ...
                      <textarea
                        ...
                        placeholder="Modify existing sections or add new ones — e.g. 'Add more detail to Past Performance' or 'Expand the Core Competencies section' or 'Add a section about our small business status'"
...
export default SourcesSoughtWriter;
