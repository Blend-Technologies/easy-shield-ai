...
serve(async (req) => {
  ...
  try {
    ...
    // Use Supabase's own DB — always reachable from edge functions, no firewall issues.
    ...
    if (!SUPABASE_DB_URL)       throw new Error("SUPABASE_DB_URL secret is not configured");
    ...
    if (!documents?.length)     throw new Error("No documents provided");
